...
# This may contain flags generated by YCM-Generator plugin or set manually.
known_flags = [
    # INSERT FLAGS HERE
]
...
def FindCompilationDatabase( wd ):
  ...
  for folder in utils.PathsToAllParentFolders( wd ):
    ...
    if os.path.exists( compile_commands ):
      ...
      return database
    # Check build folder as well.
    compile_commands = os.path.join( folder, 'build', 'compile_commands.json' )
    if os.path.exists( compile_commands ):
      database = ycm_core.CompilationDatabase( folder )
      compilation_database_dir_map[ folder ] = database
      return database
...
def FlagsForFile( file_name, **kwargs ):
  ...
  # Return flags if they are already known
